...
import {
  ...
  Building2, Heart, DollarSign, Plus,
} from 'lucide-react';
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-gray-50 last:border-0">
      ...
      <div className="flex items-center gap-2">
        {expiry && <span className="text-xs text-gray-400">Exp: {new Date(expiry).toLocaleDateString('en-PH', { year: 'numeric', month: 'short', day: 'numeric' })}</span>}
...
function HrTypeBadge({ type }) {
  const map = {
    ...
    loa:  'bg-purple-100 text-purple-700',
  };
      ...
      <div className="space-y-5">
            ...
            <div className="flex items-center gap-3 mt-1.5 text-sm text-gray-500 flex-wrap">
              {employee.position?.name && (
                <span className="flex items-center gap-1">
                  <Briefcase className="h-3.5 w-3.5" /> {employee.position.name}
                </span>
              )}
              {employee.branch?.branch_name && (
                <span className="flex items-center gap-1">
                  <Building2 className="h-3.5 w-3.5" /> {employee.branch.branch_name}
                </span>
              )}
              {employee.date_hired && (
                <span className="flex items-center gap-1">
                  <Calendar className="h-3.5 w-3.5" /> Hired {fmt(employee.date_hired)}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {!isEditing ? (
              <>
                ...
                <button
                  onClick={() => { setIsEditing(true); setShowStatusMenu(false); }}
              ...
              <>
                <button
                  onClick={() => { setIsEditing(false); initForm(employee); }}
            ...
            <SectionCard title="Basic Information" icon={User} iconColor="text-blue-500">
              {!isEditing ? (
                ...
                <div className="space-y-3">
                  ...
                  <Field label="Gender" name="gender" value={form.gender} onChange={handleChange} options={[{ value: 'Male', label: 'Male' }, { value: 'Female', label: 'Female' }]} />
        ...
        {activeTab === 'employment' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <SectionCard title="Remarks" icon={FileText} iconColor="text-gray-400">
              {!isEditing ? (
                <p className="text-sm text-gray-600 leading-relaxed">{employee.remarks || <span className="text-gray-300 italic">No remarks</span>}</p>
              ) : (
                <textarea
                  name="remarks"
                  value={form.remarks || ''}
        ...
        {activeTab === 'documents' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            {/* Government IDs view (read-only in docs tab) */}
        ...
        {activeTab === 'hr_actions' && (
          <div className="space-y-4">
            ...
            {/* HR Actions list */}
            {employee.hr_actions?.length === 0 || !employee.hr_actions ? (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          {action.subject && (
                            <p className="text-sm font-semibold text-gray-800">{action.subject}</p>
                          )}
                          {action.description && (
                          ...
                          <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                            ...
                            {action.created_by?.name && (
                              <span className="flex items-center gap-1">
                                <User className="h-3 w-3" /> {action.created_by.name}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <button
                        onClick={() => handleDeleteHrAction(action.id)}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="flex items-center gap-2">
              {[
                ...
                <button
                  key={t.value}
                  onClick={() => setHrForm((f) => ({ ...f, type: t.value }))}
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                disabled={savingHr || !hrForm.action_date}
