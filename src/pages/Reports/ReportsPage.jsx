...
import {
  ...
  XCircle, Briefcase, ArrowDown, RefreshCw,
...
export default function ReportsPage() {
 ...
 const handleExport = async () => {
  ...
  const sourceColors = ['bg-blue-500', 'bg-indigo-500', 'bg-violet-500', 'bg-sky-500', 'bg-cyan-500', 'bg-teal-500'];
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-white bg-green-600 rounded-lg hover:bg-green-700 transition shadow-sm"
            >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          {/* 4 columns: start date, end date, client, branch */}
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <KpiCard label="Active"           value={active}                icon={Briefcase}   color="yellow" sub="In pipeline" />
              ...
              <SectionCard title="Applicants by Source" icon={TrendingUp} iconColor="text-blue-500">
                {sourceData.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {sourceData.sort((a, b) => b.count - a.count).map((item, i) => (
                      <HBar key={item.source} label={item.source} count={item.count} total={totalApplicants} color={sourceColors[i % sourceColors.length]} rank={i + 1} />
                    ))}
                  </div>
                )}
              </SectionCard>
                ...
                {statusData.length === 0 ? (
                  ...
                  <>
                    <div className="space-y-4 mb-6">
                      {statusData.map((item) => {
                        const colorMap = { active: 'bg-yellow-500', hired: 'bg-green-500', rejected: 'bg-red-500', withdrawn: 'bg-gray-400' };
                        return <HBar key={item.status} label={item.status.charAt(0).toUpperCase() + item.status.slice(1)} count={item.count} total={totalApplicants} color={colorMap[item.status] || 'bg-gray-400'} />;
                      })}
                    </div>
                    <div className="grid grid-cols-2 gap-2 pt-4 border-t border-gray-50">
                      {statusData.map((item) => {
                        const pillMap = { active: 'bg-yellow-50 text-yellow-700 border-yellow-200', hired: 'bg-green-50 text-green-700 border-green-200', rejected: 'bg-red-50 text-red-700 border-red-200', withdrawn: 'bg-gray-50 text-gray-600 border-gray-200' };
                        return (
                          <div key={item.status} className={`flex justify-between items-center px-3 py-2 rounded-lg border text-xs font-semibold ${pillMap[item.status] || 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                    ...
                    <div key={item.branch_name} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-xs font-bold text-gray-500 ml-2 flex-shrink-0">
                            {item.count} <span className="text-gray-300">({maxBranch > 0 ? ((item.count / maxBranch) * 100).toFixed(0) : 0}%)</span>
                          </span>
                        </div>
                        <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
            ...
            {/* ── Row 3: Top Recruiters (full width) ── */}
                ...
                <>
                  ...
                  <div className="space-y-1">
                    {recruitersData.map((recruiter, i) => (
                      <RecruiterRow key={recruiter.id} recruiter={recruiter} rank={i + 1} maxAdded={maxAdded} />
                    ))}
                  </div>
                  <div className="mt-6 pt-4 border-t border-gray-50 grid grid-cols-3 gap-4 text-center">
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-800">{recruitersData[0]?.name || '—'}</p>
                    </div>
                    <div>
